import React from "react";
import { Position } from "@xyflow/react";
import {
  NodeCard,
  LabeledInput,
  LabeledSelect,
  VectorInput,
  InfoCard,
  HandleWithLabel,
} from "./components";

const BROADCAST_OPTIONS = [
  { value: "static",  label: "Static (tf_static)" },
  { value: "dynamic", label: "Dynamic (tf)" },
];

const XYZ_KEYS = ["x", "y", "z"];
const RPY_KEYS = ["roll", "pitch", "yaw"];

/**
 * TF Child Node — a child frame whose pose is expressed relative to its parent.
 */
export default function TfChildNode({ id, data }) {
  const d = data || {};
  const edit = (patch) => d.onChange?.(id, patch);

  const position = d.position || { x: 0, y: 0, z: 0 };
  const rotation = d.rotation || { roll: 0, pitch: 0, yaw: 0 };

  return (
    <NodeCard
      title={<>&#x1f517; TF Child Frame</>}
      size="lg"
      handles={
        <>
          <HandleWithLabel type="target" position={Position.Left}  id="parent_frame"    label="parent" color="cyan"   top="30%" />
          <HandleWithLabel type="target" position={Position.Left}  id="center_mass"     label="center" color="orange" top="60%" />
          <HandleWithLabel type="source" position={Position.Right} id="child_frame_out" label="child"  color="cyan" />
        </>
      }
    >
      <LabeledInput
        label="Child Frame ID"
        value={d.childFrameId || ""}
        onChange={(v) => edit({ childFrameId: v })}
        placeholder="child_frame"
      />

      <VectorInput
        label="Relative Position (m)"
        value={position}
        keys={XYZ_KEYS}
        onChange={(next) => edit({ position: next })}
        placeholders={["X", "Y", "Z"]}
      />

      <VectorInput
        label="Relative Rotation (rad)"
        value={rotation}
        keys={RPY_KEYS}
        onChange={(next) => edit({ rotation: next })}
        placeholders={["Roll", "Pitch", "Yaw"]}
      />

      <LabeledSelect
        label="Broadcast Type"
        value={d.broadcastType || "static"}
        onChange={(v) => edit({ broadcastType: v })}
        options={BROADCAST_OPTIONS}
      />

      <InfoCard tone="blue">
        Position and rotation are relative to parent frame
      </InfoCard>
    </NodeCard>
  );
}
